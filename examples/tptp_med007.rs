//! MED007 - Medical domain conjecture
//!
//! A KK encoding of MED007+1.p from http://www.cs.miami.edu/~tptp/
//!
//! Following Java: kodkod.examples.tptp.MED007
...
impl MED007 {
    ...
    pub fn transsls2_qilt27(&self) -> Formula {
    ...
    /// axioms() && !transsls2_qilt27()
    pub fn check_transsls2_qilt27(&self) -> Formula {
        self.base.axioms().and(self.transsls2_qilt27().not())
    }
}

impl Default for MED007 {
    fn default() -> Self {
        Self::new()
    }
}
...
fn usage() -> ! {
    eprintln!("Usage: tptp_med007 <scope>");
    std::process::exit(1);
}
...
fn main() -> Result<(), kodkod_rs::error::KodkodError> {
    ...
    let model = MED007::new();
    let formula = model.check_transsls2_qilt27();
    let bounds = model.base.bounds(n)?;

    println!("=== MED007 (scope={n}) ===\n");
